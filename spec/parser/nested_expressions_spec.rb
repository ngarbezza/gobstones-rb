describe Gobstones::Parser, "nested expressions" do
  ...
  it "should parse literals between ()" do
    '(  42 )'.should be_parsed_to(ParenthesesExpr.new(Number.new(42)))
    '(True )'.should be_parsed_to(ParenthesesExpr.new(True.new))
    '(Rojo )'.should be_parsed_to(ParenthesesExpr.new(Rojo.new))
    '(Verde)'.should be_parsed_to(ParenthesesExpr.new(Verde.new))
  end

  it "should parse primitive functions between ()" do
    puede_mover = ParenthesesExpr.new(PuedeMover.new(Norte.new))
    '(puedeMover(Norte))'.should be_parsed_to(puede_mover)
  end
  ...
  it "should parse nested arithmetic expressions between ()" do
    ...
    '(3 + 5) * 6'.should be_parsed_to(expected)
  end
  ...
  it "should parse a nested expressions with many ()" do
    ...
    '(a && (b || c)) && d'.should be_parsed_to(result)
  end
